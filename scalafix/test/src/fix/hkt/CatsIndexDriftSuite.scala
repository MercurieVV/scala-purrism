...
final class CatsIndexDriftSuite extends FunSuite {
  ...
  private val headers = Map(
    ...
    "stdlib.tsv" ->
      List("concreteMethod", "kind", "owner", "method", "note"),
    "gaps.tsv" ->
      List("typeclass", "reason", "tracked")
  )

  test("regeneration is byte-identical to every generator-owned TSV") {
    val generated = Paths.get(testkitProperties("catsIndexGenerated"))
    headers.keys.filterNot(_ == "stdlib.tsv").foreach { fileName =>
      val actual = Files.readAllBytes(generated.resolve(fileName))
      assertEquals(
        actual.toSeq,
        resourceBytes(fileName).toSeq,
        s"Cats index drift in $fileName; run: rtk mill scalafix.catsIndex"
      )
    }
  }

  test("stdlib capability targets and decline names are audited") {
    val capabilityTargets = load("capabilities.tsv").rows.iterator
      .map(row => row(2) -> row(1))
      .toSet
    val declineNames =
      Set(
    ...
    load("stdlib.tsv").rows.foreach { row =>
      row(1) match {
        case "capability" =>
          assert(
            capabilityTargets(row(2) -> row(3)),
            s"stdlib capability row has unresolved target: ${row.mkString("\t")}"
          )
        case "decline" =>
          assert(
            declineNames(row(3)),
            s"stdlib decline row has invalid reason: ${row.mkString("\t")}"
          )
          assert(
            row(2).isEmpty,
  ...
  test("stdlib is byte-identical to its canonical re-serialisation") {
    ...
    assertEquals(
      serialised.getBytes(StandardCharsets.UTF_8).toSeq,
      resourceBytes(fileName).toSeq,
      s"$fileName differs from canonical stable-sort serialisation"
    )
  }

  test("duplicate stdlib concrete methods are capability ambiguities only") {
    load("stdlib.tsv").rows
      .groupBy(_.head)
      .foreach { case (concreteMethod, rows) =>
        if (rows.size > 1)
          assert(
            rows.forall(_(1) == "capability"),
            s"duplicate stdlib method is not capability-only: $concreteMethod: " +
              rows.map(_.mkString("\t")).mkString(" | ")
          )
      }
  }
  ...
  test("all Cats index files have canonical headers, rows, and bytes") {
    headers.foreach { case (fileName, expectedHeader) =>
      ...
      table.rows
        .zip(table.rows.drop(1))
        .find { case (left, right) => compareRows(left, right) >= 0 }
        .foreach { case (left, offending) =>
          fail(
            s"$fileName row is not strictly stable-sorted: " +
              s"${offending.mkString("\t")}; previous row: ${left.mkString("\t")}"
          )
        }
      assertEquals(
  ...
  private def parse(fileName: String, text: String): Table = {
    ...
    if (fileName == "stdlib.tsv")
      assertEquals(
        content.lift(1),
        Some("# hand-written; audited by CatsIndexDriftSuite"),
        s"$fileName lacks its hand-written audit header"
      )
    else {
      assert(
        content.lift(1).exists(_.startsWith("# generated by CatsIndexGen;")),
        s"$fileName lacks its generated-source header"
      )
      assert(
        content(1).contains("source: org.typelevel::cats-core:") &&
          content(1).contains(" for Scala ") &&
          content(1).endsWith("; extraction: TASTy"),
        s"$fileName lacks its coordinate or TASTy extraction route"
      )
    }
    Table(
      content.head.drop(1).split("\t", -1).toList,
      content.dropWhile(_.startsWith("#")).map(_.split("\t", -1).toList)
    )
  }

  private lazy val testkitProperties: Map[String, String] = {
    val stream = Option(
      getClass.getClassLoader.getResourceAsStream("scalafix-testkit.properties")
    ).getOrElse(fail("missing scalafix-testkit.properties"))
    try {
