...
Match the JBR build to the platform's `runtimeBuild` in `<platformHome>/dependencies.txt`. If unset, `runIde` falls back to `sys.props("java.home")` (whatever JVM Mill runs on) — JCEF may not work there, and on macOS the plain-JDK fallback also crashes the native menu bar (`NoSuchMethodError` on `sun.lwawt.macosx.LWCToolkit`) since IntelliJ's screen-menu bridge only exists in JBR.

`run-ide.sh` (generated by `genRunIdeScript`, see below) doesn't need `PURRISM_JBR_HOME` set every time: at run time it checks the env var first, then auto-discovers the newest `~/.cache/purrism-jbr/jbr_jcef-*/Contents/Home`, and only falls back to `sys.props("java.home")` if neither exists. `mill ijPlugin.runIde` itself still needs the env var explicitly (it bakes in `sys.props("java.home")` at Mill-task run time, before any script logic runs).

`run-ide.sh` also rebuilds the plugin itself (`mill ijPlugin.pluginDist`) as its first step, so a plain rerun always picks up source changes — no separate `mill ijPlugin.pluginDist` needed first. That rebuild briefly needs Mill's workspace lock, same as any other `mill` command.
