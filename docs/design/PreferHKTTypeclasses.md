...
`stdlib.tsv` is the one hand-written file — the map from concrete stdlib/Cats-data
methods (`scala/collection/immutable/List#map().`) to the capability they demand. It
is small, it is reviewed, and it is *audited* by item 8 rather than generated.
...
2. data rows sorted **ascending** by column 1, ties broken by column 2, then column 3,
   … through the last column;
3. comparison is `java.lang.String.compareTo` on the raw cell text — UTF-16 code-unit
...
5. no timestamps, no absolute paths, no jar hashes anywhere in the file.
