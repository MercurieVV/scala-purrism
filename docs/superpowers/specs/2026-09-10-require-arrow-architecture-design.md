...
`allowedConcreteTypePatterns` (default empty) is a list of regexes, each
tested against a type's fully-qualified symbol (e.g. `"^scala/Int#$"`),
naming concrete types that may appear as the value types a slot is applied
to (the `A`/`B` in `Step[A, B]`) — see "Arrow slot arguments" below for
what's allowed by default without any config at all.
...
applied to two types, e.g. `Step[Int, Either[Error, Int]]`, **and each of
those two types is itself allowed** — see "Arrow slot arguments" below;
this is not automatic just because the slot itself is abstract. **Naming a
concrete arrow implementation directly — `Int => Int`, `Kleisli[F, A, B]`,
or any other type with its own name rather than the slot's — is a
violation**, even though such a type would itself satisfy
...
### Arrow slot arguments

The two types a slot is applied to (the `A`/`B` in `Step[A, B]`) aren't a
free-for-all just because `Step` itself is abstract — a member typed
`Step[Int, Int]` still names two concrete types, and by default that's as
much a commitment to specifics as naming a concrete arrow would be. What's
allowed, checked **recursively** (every concrete type name found anywhere
inside the argument must be allowed, not just the outermost one):

- an abstract type parameter or abstract type member in scope wherever the
  argument is written (a class/method type parameter, or a `type X`
  member) — this is the common case: `trait Generic[Step[_, _]: Arrow, A,
  B] { def run: Step[A, B] }` needs no configuration at all,
- a Scala 3 literal tuple of any arity, e.g. `(A, Int)` — allowed as a
  wrapper by default, with each element still checked,
- `Either[_, _]` or `Option[_]` — likewise allowed by default as a
  wrapper, with their own type arguments still checked,
- a type whose fully-qualified symbol matches one of the configured
  `allowedConcreteTypePatterns` regexes (e.g. `Int`, `String`, or a
  project's own domain types) — again allowed as a wrapper if it's itself
  applied to further arguments, which are still checked.

Anything else — a concrete type not covered by the above — is a violation,
reported on the enclosing member/param with a message distinct from "not a
slot application at all" (the shape is right, one argument isn't). The
default (`allowedConcreteTypePatterns = []`) means the only concrete types
usable inside a scoped file at all are the structural wrappers above; a
team writing real leaf types like `Int`/`String`/domain IDs into their
architecture code opts in explicitly, one pattern at a time.
...
Both examples below assume `allowedConcreteTypePatterns = ["^scala/Int#$",
"^scala/Predef\\.String#$"]` is configured, so `Int`/`String` may appear as
slot arguments; without it, every `Step[Int, ...]` below would additionally
violate the "Arrow slot arguments" rule.
...
- Cases to cover: a conforming trait/case-class/object built on an abstract
  ...
  a slot applied only to the enclosing definition's own abstract type
  parameters (conforming, no config needed); a slot argument matching a
  configured `allowedConcreteTypePatterns` entry (conforming) versus the
  same shape with the pattern absent (violation); `Either`/`Option`/a
  tuple used as a slot argument with an allowlisted leaf type nested
  inside (conforming) versus a non-allowlisted leaf type nested inside one
  of those wrappers (violation, since the check is recursive).
