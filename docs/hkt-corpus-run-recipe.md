...
`PreferHKTTypeclasses`, the pinned consumption recipe, and copy‑pasteable command
...
with `PreferHKTTypeclasses` registered and its fixtures exercised.

## Verdict

- [ ] **GO** – `rtk mill scalafix.test` is green and the rule is exercised.
- [ ] **BLOCKED** – see failure output below.
